...
# 56 "../canfd.h"
 ...
 typedef struct {
  volatile uint8_t blob[64];
  volatile _Bool tx_flag, rx_flag;
 } blob_type;
 ...
 extern char info_buffer[512], log_buffer[512];
 void can_fd_tx(void);
 void can_setup(void);
...
CAN_MSG_OBJ msg[3];
volatile uint8_t rxMsgData[4][64] = {
 " no data            ",
 " no_data0           ",
 " no info1           ",
 " no info2           ",
...
void Can1FIFO1NotEmptyHandler(void)
{
 ...
 while (1) {
  ...
  if (CAN1_ReceiveFrom(FIFO1, &msg[half]))
  {
   memcpy(&msg[3 -1], &msg[half], sizeof(CAN_MSG_OBJ));
   if ((msg[half].msgId & 0xf) == 0x2 || (msg[half].msgId & 0xf) == 0x3) {
    memcpy((void *) &rxMsgData[half][0], msg[half].data, 64);
    if ((msg[half].msgId & 0xf) == 0x2) {
     half = 1;
     break;
    }
    if ((msg[half].msgId & 0xf) == 0x3) {
     half = 0;
     can_rec_count.rec_flag = 1;
     break;
    }
   }
   if ((msg[half].msgId & 0xf) == 0xC) {
    memcpy((void *) &rxMsgData[2][0], msg[half].data, 64);
   ...
   if ((msg[half].msgId & 0xf) == 0xF) {
    memcpy((void *) &rxMsgData[3][0], msg[half].data, 64);
    break;
   }
   if ((msg[half].msgId & 0xf) == 0xD) {
    memcpy((void *) &blob_rec, msg[half].data, 64);
    blob_rec.rx_flag = 1;
    break;
   }
   if ((msg[3 -1].msgId & 0xf) == 0xA) {
    memcpy((void *) &can_timer, msg[3 -1].data, sizeof(time_t));
...
void can_fd_tx(void)
{
 ...
 Transmission.field.idType = CAN_FRAME_EXT;
 Transmission.msgId = 0x2;
 Transmission.data = (uint8_t*) log_buffer;
 if (CAN_TX_FIFO_AVAILABLE == (CAN1_TransmitFIFOStatusGet(TXQ) & CAN_TX_FIFO_AVAILABLE))
 {
  CAN1_Transmit(TXQ, &Transmission);
 }
 Transmission.msgId = (0x3);
 Transmission.data = (uint8_t*) log_buffer + 64;
 if (CAN_TX_FIFO_AVAILABLE == (CAN1_TransmitFIFOStatusGet(TXQ) & CAN_TX_FIFO_AVAILABLE))
 {
  CAN1_Transmit(TXQ, &Transmission);
 }
 Transmission.msgId = (0xD + (B.node_id << 4));
 Transmission.data = (uint8_t*) & blob;
 if (CAN_TX_FIFO_AVAILABLE == (CAN1_TransmitFIFOStatusGet(FIFO3) & CAN_TX_FIFO_AVAILABLE))
 {
  CAN1_Transmit(FIFO3, &Transmission);
 }
 blob.tx_flag = 0;

 Transmission.msgId = (0xF);
 Transmission.data = (uint8_t*) info_buffer;
 if (CAN_TX_FIFO_AVAILABLE == (CAN1_TransmitFIFOStatusGet(FIFO3) & CAN_TX_FIFO_AVAILABLE))
 {
  CAN1_Transmit(FIFO3, &Transmission);
 }
...
void can_fd_lcd_mirror(const uint8_t r, char *strPtr)
{
 ...
 if (!B.FM80_io) {
  ...
  Transmission.field.frameType = CAN_FRAME_DATA;
  Transmission.field.idType = CAN_FRAME_EXT;
  Transmission.msgId = 0x4 + r + (B.node_id << 4);
  Transmission.data = (uint8_t*) strPtr;
  if (CAN_TX_FIFO_AVAILABLE == (CAN1_TransmitFIFOStatusGet(TXQ) & CAN_TX_FIFO_AVAILABLE))
  {
   CAN1_Transmit(TXQ, &Transmission);
  }
