...
const char *build_date = "Dec  2 2024", *build_time = "07:59:16";
...
void main(void)
{
 ...
 init_display();
 snprintf(buffer, 512, "%s   ", build_version);
  ...
  if (TimerDone(TMR_SPIN)) {
   {
    ...
    if (C.data_ok && (M.error > error_save)) {
     ...
     if (C.cstate == RECV || C.cstate == CLEAR) {
      if (B.alt_display == 0) {
       uint16_t vt, vw, vf;
       M.error = 0;
      ...
      if (B.alt_display > 0) {
       ...
       eaDogM_WriteStringAtPos(1, 0, buffer);
       snprintf(buffer, 512, "CK ER %d %d %d               ", C.crc_err, C.req_length, M.recv_count);
       if (C.dcu_online) {
        C.dcu_online = 0;
        buffer[19] = spinners(4, 0);
       }
