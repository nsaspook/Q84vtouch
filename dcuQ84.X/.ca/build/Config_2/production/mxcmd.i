...
static volatile uint16_t tbuf[32], rbuf[32];
...
{
 if (dcount == 0) {
  memcpy((void *) tbuf, (const void *) data, (size_t) (count << 2));
  dstart = 0;
  dcount = count;
 }
 return dstart;
...
void FM_io(void)
{
 ...
 if (pace++ > 2) {
  if (dcount-- > 0) {
   if (tbuf[dstart] > 0xff) {
    U1P1L = (uint8_t) tbuf[dstart];
   } else {
    UART1_Write((uint8_t) tbuf[dstart]);
   }
   dstart++;
  } else {
   dstart = 0;
...
uint8_t FM_rx(uint16_t * data)
{
 ...
 (INTCON0bits.GIEH = 0);
 count = rdstart;
 memcpy(data, (const void *) rbuf, (size_t) (count << 2));
 rdstart = 0;
 (INTCON0bits.GIEH = 1);
